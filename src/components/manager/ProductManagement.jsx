  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const parsedSizes = form.sizes.map(s => {
        const origSize = editingProduct?.sizes?.find(os => os.name === s.name);
        return {
          name: s.name.trim(),
          price: parseFloat(s.price) || 0,
          cost: parseFloat(s.cost) || 0,
          ingredient_ids: s.ingredient_ids || origSize?.ingredient_ids || []
        };
      });

      await saveProduct({
        ...form,
        ingredient_ids: editingProduct?.ingredient_ids || form.ingredient_ids || [],
                  ...
                  <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-3">
                      <div className="flex flex-col gap-1 items-start">
                        ...
                        >
                          {checkProductAvailability(p, products, ingredients) ? 'In Stock' : 'Unavailable'}
                        </button>
