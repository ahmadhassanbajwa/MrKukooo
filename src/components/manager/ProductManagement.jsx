...
export default function ProductManagement({
  ...
  const handleOpenModal = (prod = null) => {
    ...
    if (prod) {
      ...
      setForm({
        ...
        sizes: (prod.sizes || []).map(s => ({ ...s, price: s.price.toString() })),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert("Failed to save product. If you are not connected to Firebase, your browser's local storage might be full. Error: " + err.message);
    } finally {
            ...
            <div className="p-6 overflow-y-auto space-y-4 text-xs font-semibold text-accent flex-1">
              ...
              <div className="bg-gray-50/50 p-4 rounded-2xl border border-gray-150 space-y-4">
                ...
                {form.is_deal ? (
                  ...
                  <div className="space-y-1">
                    <label className="block text-[10px] font-black uppercase text-gray-400">Initial Quantity</label>
                    <input
                      type="number"
                      placeholder="e.g. 50"
                      value={form.quantity}
                      onChange={(e) => setForm({ ...form, quantity: e.target.value })}
                      className="w-full bg-white border border-gray-200 px-4 py-2 rounded-xl font-bold focus:outline-none focus:ring-1 focus:ring-secondary/50 text-xs"
                    />
                  </div>
                )}
              </div>
              ...
              <div className="bg-gray-50/50 p-4 rounded-2xl border border-gray-150 space-y-4">
                ...
                {form.has_sizes && (
                  <div className="space-y-2">
                    ...
                    <button
                      type="button"
                      onClick={() => setForm({ ...form, sizes: [...form.sizes, { name: '', price: '' }] })}
                      className="text-[10px] font-black text-secondary hover:underline flex items-center gap-1 uppercase tracking-wider"
                    >
