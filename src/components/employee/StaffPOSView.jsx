...
import { Bike, ShoppingBag, Trash2, Plus, Minus, ArrowLeft, Utensils, X, MapPin, Ticket, Percent, DollarSign } from 'lucide-react';
import { checkProductAvailability, checkSizeAvailability, getAvailableQuantity } from '../../utils/productUtils';
...
export default function StaffPOSView({
  ...
  // POS Direct Discount State (applies to all 3 order types)
  const [posDiscountType, setPosDiscountType] = useState('price'); // 'price' | 'percentage'
  ...
  // Live discount amount computation
  const discountAmount = useMemo(() => {
    const val = parseFloat(posDiscountValue);
    if (isNaN(val) || val <= 0) return 0;
    return calculateDiscount(posDiscountType, val, cartTotal);
  }, [posDiscountType, posDiscountValue, cartTotal]);
  ...
  const handleCheckoutSubmit = (e) => {
    ...
    // ── Inventory stock & size availability gate ─────────────────────
    for (const item of cart) {
      const prod = (products || []).find(p => p.id?.toString() === (item.product_id || item.id)?.toString());
      if (!prod || !prod.is_available) {
        alert(`Cannot place order: "${item.name}" is currently unavailable.`);
        return;
      }
      if (item.size && prod.has_sizes && prod.sizes) {
        ...
        if (!matchingSize || !checkSizeAvailability(prod, matchingSize, ingredients)) {
          alert(`Cannot place order: "${prod.name} (${item.size.name})" is unavailable.`);
          return;
        }
        const avail = getAvailableQuantity(prod, matchingSize, ingredients, products);
        if (item.quantity > avail) {
          alert(`Only ${avail} available for "${prod.name} (${item.size.name})". Ticket has ${item.quantity}.`);
          return;
        }
      } else {
        if (!checkProductAvailability(prod, products, ingredients)) {
          alert(`Cannot place order: "${prod.name}" is unavailable.`);
          return;
        }
        const avail = getAvailableQuantity(prod, null, ingredients, products);
        if (item.quantity > avail) {
          alert(`Only ${avail} available for "${prod.name}". Ticket has ${item.quantity}.`);
          return;
        }
      }
    }
    ...
    try {
      ...
      const finalTotal = Math.max(0, discountedTotal + deliveryFee);
      ...
      const newOrder = {
        ...
        discount_type: discountAmount > 0 ? posDiscountType : null,
        discount_value: discountAmount > 0 ? (parseFloat(posDiscountValue) || 0) : 0,
  ...
  const renderTicketContent = () => (
    <>
          ...
          <div className="space-y-2.5 sm:space-y-3">
            {cart.map(item => {
              ...
              if (prod) {
                if (item.size && prod.has_sizes && prod.sizes) {
                  const matchingSize = prod.sizes.find(s => s.name === item.size.name);
                  isAvailable = !!(matchingSize && checkSizeAvailability(prod, matchingSize, ingredients));
                  availableQty = matchingSize ? getAvailableQuantity(prod, matchingSize, ingredients, products) : 0;
                } else {
                  isAvailable = checkProductAvailability(prod, products, ingredients);
                  availableQty = getAvailableQuantity(prod, null, ingredients, products);
                }
              }
              const hasInsufficientStock = isAvailable && item.quantity > availableQty;

              return (
                <div key={item.cartItemId} className={`border-2 rounded-xl p-2.5 sm:p-3 relative comic-shadow-sm ${!isAvailable ? 'bg-red-50/50 border-red-200' : hasInsufficientStock ? 'bg-amber-50/50 border-amber-200' : 'bg-white border-gray-100'}`}>
                  <div className="flex justify-between items-start font-bold mb-1 text-accent text-xs sm:text-sm">
                    <div className="flex items-center gap-1.5 flex-wrap min-w-0 pr-2">
                      <span className="truncate font-heading font-black">{item.name}</span>
                      {!isAvailable && (
                        <span className="text-[10px] font-black text-red-600 bg-red-100 border border-red-200 px-1.5 py-0.5 rounded">
                          Unavailable
                        </span>
                      )}
                  ...
                  <div className="flex justify-between items-center mt-1.5">
                    <div className="flex items-center gap-1 bg-gray-50 rounded-lg p-0.5 border border-gray-200">
                      <button 
                        ...
                        className="w-7 h-7 flex items-center justify-center bg-white hover:bg-gray-100 rounded text-accent comic-shadow-sm active:scale-95 cursor-pointer"
                      >
                      ...
                      <button 
                        ...
                        className="w-7 h-7 flex items-center justify-center bg-white hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed rounded text-accent comic-shadow-sm active:scale-95 cursor-pointer"
                      >
                    ...
                    <button 
                      ...
                      className="text-gray-400 hover:text-primary transition-colors p-1.5 bg-gray-50 hover:bg-red-50 rounded-lg cursor-pointer"
                      title="Remove item"
                    >
      ...
      <div className="bg-white p-3 sm:p-4 border-t-2 border-gray-100 shrink-0 space-y-2.5">
        {/* Direct Discount Field for All 3 Order Types */}
        <div className="bg-gray-50 p-2.5 rounded-xl border border-gray-200 space-y-1.5">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-black text-gray-600 uppercase tracking-wider flex items-center gap-1">
              <Ticket className="w-3 h-3 text-secondary" /> Discount
            </span>
            {discountAmount > 0 && (
              <span className="text-[10px] font-black text-emerald-700 bg-emerald-100 border border-emerald-300 px-1.5 py-0.2 rounded">
                -Rs. {discountAmount} {posDiscountType === 'percentage' ? `(${Math.min(100, Number(posDiscountValue))}% off)` : 'off'}
              </span>
            )}
          </div>
          <div className="flex items-center gap-1.5">
            <div className="flex bg-white border border-gray-300 rounded-lg p-0.5 shrink-0">
              <button
                type="button"
                onClick={() => setPosDiscountType('price')}
                className={`px-2 py-1 text-xs font-black rounded-md transition-all cursor-pointer ${
                  posDiscountType === 'price' ? 'bg-secondary text-accent shadow-2xs' : 'text-gray-400 hover:text-gray-700'
                }`}
                title="Fixed Amount Discount (Rs.)"
              >
                Rs
              </button>
              <button
                ...
                className={`px-2 py-1 text-xs font-black rounded-md transition-all cursor-pointer ${
                  posDiscountType === 'percentage' ? 'bg-secondary text-accent shadow-2xs' : 'text-gray-400 hover:text-gray-700'
                }`}
            ...
            <input
              ...
              placeholder={posDiscountType === 'percentage' ? 'Discount % (e.g. 10)' : 'Discount Rs. (e.g. 100)'}
              value={posDiscountValue}
              onChange={(e) => setPosDiscountValue(e.target.value)}
              className="flex-1 min-w-0 bg-white border border-gray-300 px-2.5 py-1.5 rounded-lg text-xs font-bold text-accent focus:outline-none focus:border-primary"
            />
            {posDiscountValue && (
              <button
                type="button"
                onClick={() => setPosDiscountValue('')}
                className="text-gray-400 hover:text-red-500 px-1 text-xs font-black cursor-pointer"
        ...
        <div className="space-y-1">
          ...
          {discountAmount > 0 && (
            <div className="flex justify-between items-center text-xs text-emerald-700 font-black">
              <span>Discount:</span>
          ...
          <div className="flex justify-between items-end pt-1">
            ...
            <span className="text-primary font-black font-heading text-2xl sm:text-3xl">Rs. {discountedTotal.toFixed(2)}</span>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden comic-shadow-xl animate-scale-up flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleCheckoutSubmit} className="p-6 flex-1 overflow-y-auto space-y-4">
              ...
              {/* Discount Input for All 3 Order Types (Dine-In, Takeaway, Delivery) */}
              <div className="bg-gray-50 p-3 rounded-xl border border-gray-200 space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-accent font-bold text-xs uppercase tracking-wider flex items-center gap-1.5">
                    <Ticket className="w-3.5 h-3.5 text-secondary" /> Order Discount
                  </label>
                  {discountAmount > 0 && (
                    <span className="text-[11px] font-black text-emerald-700 bg-emerald-100 border border-emerald-300 px-2 py-0.5 rounded">
                      -Rs. {discountAmount} {posDiscountType === 'percentage' ? `(${Math.min(100, Number(posDiscountValue))}% off)` : 'off'}
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  <div className="flex bg-white border border-gray-300 rounded-lg p-0.5 shrink-0 shadow-2xs">
                    <button
                      type="button"
                      onClick={() => setPosDiscountType('price')}
                  ...
                  <input
                    ...
                    min="0"
                    max={posDiscountType === 'percentage' ? '100' : undefined}
                    placeholder={posDiscountType === 'percentage' ? 'Enter discount % (e.g. 10)' : 'Enter discount Rs. (e.g. 100)'}
                    value={posDiscountValue}
                    onChange={(e) => setPosDiscountValue(e.target.value)}
                    className="flex-1 bg-white border border-gray-300 px-3 py-2 rounded-xl text-xs font-bold text-accent focus:outline-none focus:border-secondary"
                  />
                  {posDiscountValue && (
                    <button
                      type="button"
                      onClick={() => setPosDiscountValue('')}
                      className="text-gray-400 hover:text-red-500 px-2 text-xs font-black cursor-pointer"
                      title="Clear discount"
                    >
                      ✕
                    </button>
              ...
              <div className="pt-4 border-t-2 border-gray-100 mt-6 space-y-2">
                <div className="space-y-1 text-xs font-bold text-gray-600">
                  ...
                  {discountAmount > 0 && (
                    <div className="flex justify-between text-emerald-700 font-black">
                      <span>Discount ({posDiscountType === 'percentage' ? `${posDiscountValue}%` : `Rs. ${posDiscountValue}`}):</span>
                      <span>- Rs. {discountAmount.toFixed(2)}</span>
                    </div>
                  )}
                  {checkoutModal.type === 'Delivery' && (
                    <div className="flex justify-between text-gray-700">
                      ...
                      <span>Rs. {Number(customerDetails.deliveryCharges) || 0}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-between items-center pt-2 border-t border-gray-200">
                  ...
                  <span className="font-heading font-black text-2xl text-primary">
                    Rs. {(discountedTotal + (checkoutModal.type === 'Delivery' ? (Number(customerDetails.deliveryCharges) || 0) : 0)).toFixed(2)}
                  </span>
                </div>
                <button
                  type="submit"
                  className="w-full bg-secondary hover:bg-secondary-hover text-accent font-black font-heading text-lg py-4 rounded-xl comic-shadow-sm comic-hover uppercase tracking-widest transition-colors cursor-pointer"
                >
                  Confirm Order
                </button>
              </div>
