      ...
      <header className="bg-accent text-white px-6 py-4 border-b-4 border-secondary flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[10px] text-white/50 font-bold uppercase tracking-wider">Live Order Manager</span>
          </div>
        </div>
                  ...
                  {sortedList.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {sortedList.map((order) => (
                        <div 
                          key={order.order_id} 
                          className={`bg-white rounded-2xl comic-border p-5 space-y-4 comic-shadow-sm comic-hover relative ${
                            ...
                            {order.status === 'Completed' && (
                              <div className="space-y-2.5">
                                ...
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="w-full bg-blue-600 text-white font-black py-2 rounded-xl comic-border-sm text-xs flex items-center justify-center gap-1.5 comic-shadow-sm comic-hover cursor-pointer transition-all"
                                >
                                  <ThumbsUp className="w-3.5 h-3.5" /> Get Feedback (WhatsApp)
                                </a>
                              </div>
                            )}
