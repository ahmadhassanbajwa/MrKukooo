      ...
      {activeOffers.map((offer, idx) => (
        ...
        >
          ...
          <div className="relative z-10 h-full flex flex-col justify-center p-6 sm:p-12 max-w-xl space-y-3 sm:space-y-4">
            <span className="inline-block w-fit bg-secondary text-accent text-[11px] sm:text-xs font-semibold px-2.5 py-1 rounded-md uppercase tracking-wider shadow-2xs">
              Special Promo Offer
            </span>
            <h2 className="text-2xl sm:text-4xl md:text-5xl font-bold tracking-tight leading-tight text-white drop-shadow-sm">
              {offer.title}
            </h2>
            {offer.redirect_type && offer.redirect_type !== 'none' && (
              <button className="inline-flex items-center gap-2 px-4.5 py-2.5 bg-primary hover:bg-primary-hover active:translate-y-[0.5px] text-white text-xs sm:text-sm font-medium rounded-md shadow-2xs transition-all w-fit cursor-pointer border border-primary-hover/50">
                Grab Deal Now <ArrowRight className="w-4 h-4 stroke-[2.2]" />
              </button>
            )}
          </div>
        </div>
