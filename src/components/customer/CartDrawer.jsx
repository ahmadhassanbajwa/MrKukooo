...
import { ShoppingCart, Plus, Minus, Trash2, X, MapPin, Phone, User, ShoppingBag, ArrowRight, AlertCircle, Ticket } from 'lucide-react';
import MapPicker from './MapPicker';
import { isRestaurantOpen, getHours } from '../../utils/restaurantHours';
import { generateDailyOrderId } from '../../utils/orderId';
import { checkSizeAvailability, checkProductAvailability, getAvailableQuantity } from '../../utils/productUtils';
...
export default function CartDrawer({
  ...
  const isCustomerVoucherFieldEnabled = !!voucherSettings?.customer_vouchers_enabled;

  // Dynamic discount calculation
  const discountAmount = useMemo(() => {
    if (!appliedVoucher?.voucher || !isCustomerVoucherFieldEnabled) return 0;
    return calculateDiscount(appliedVoucher.voucher.type, appliedVoucher.voucher.value, subtotal);
  }, [appliedVoucher, subtotal, isCustomerVoucherFieldEnabled]);

  const grandTotal = Math.max(0, subtotal - discountAmount + (orderType === 'Delivery' ? deliveryFee : 0));

  const handleApplyVoucher = (e) => {
    if (e) e.preventDefault();
    setVoucherError('');
    const result = validateVoucher(voucherInput, vouchers, subtotal, isCustomerVoucherFieldEnabled);
    if (!result.valid) {
  ...
  const handleCheckoutSubmit = async (e) => {
    ...
    for (const item of cart) {
      ...
      if (!prod || !prod.is_available) {
        setFormErrors({ stock: `"${item.name}" is currently unavailable.` });
        return;
      }
      if (item.size && prod.has_sizes && prod.sizes) {
        ...
        if (!matchingSize || !checkSizeAvailability(prod, matchingSize, ingredients)) {
          setFormErrors({ stock: `"${prod.name} (${item.size.name})" is unavailable. Please remove it to proceed.` });
          return;
        }
        const avail = getAvailableQuantity(prod, matchingSize, ingredients, products);
        if (item.quantity > avail) {
          setFormErrors({ stock: `Only ${avail} available for "${prod.name} (${item.size.name})". Please reduce quantity.` });
          return;
        }
      } else {
        if (!checkProductAvailability(prod, products, ingredients)) {
          setFormErrors({ stock: `"${prod.name}" is unavailable. Please remove it to proceed.` });
          return;
        }
        const avail = getAvailableQuantity(prod, null, ingredients, products);
        if (item.quantity > avail) {
          setFormErrors({ stock: `Only ${avail} available for "${prod.name}". Please reduce quantity.` });
          return;
        }
      }
    }
    ...
    const newOrder = {
      ...
      discount_type: appliedVoucher?.voucher?.type || null,
      discount_value: appliedVoucher?.voucher?.value || 0,
      ...
      <div className={innerClass}>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-5 bg-gray-50/50">
          {cart.length === 0 ? (
            ...
            <>
                ...
                <div className="space-y-2.5">
                  {cart.map((item) => {
                    ...
                    return (
                      <div
                        key={item.cartItemId}
                        className={`flex items-center gap-3 p-3 rounded-lg border shadow-2xs transition-all ${
                          !isAvailable
                        ...
                        <div className="flex flex-col items-end gap-2">
                          ...
                          <div className="flex items-center gap-1.5 bg-gray-50 border border-gray-200 px-2 py-0.5 rounded-md">
                            ...
                            <button
                              ...
                              className={`w-5 h-5 flex items-center justify-center bg-white hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed text-gray-700 font-bold rounded-sm transition-colors cursor-pointer active:scale-95 shadow-2xs`}
                            >
              ...
              {/* Promo Code / Discount Voucher Section (Only shown if enabled by Manager) */}
              {isCustomerVoucherFieldEnabled && (
                <div className="bg-white border border-gray-200 rounded-lg p-3.5 shadow-2xs space-y-2.5">
                  ...
                  {appliedVoucher ? (
                    <div className="flex items-center justify-between p-2.5 bg-emerald-50/80 border border-emerald-200 rounded-md">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-emerald-700 font-medium">
                          {appliedVoucher.voucher.type === 'percentage'
                            ...
                            : `Rs. ${appliedVoucher.voucher.value} OFF`}
                    ...
                    <div className="space-y-1.5">
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 bg-white border border-gray-250 focus:border-primary px-3 py-2 rounded-md font-mono text-xs uppercase tracking-wider text-gray-900 placeholder:normal-case placeholder:font-sans focus:outline-none focus:ring-1 focus:ring-primary/20"
                        />
                        <button
                          type="button"
                          onClick={handleApplyVoucher}
                          className="px-3.5 py-2 bg-gray-900 hover:bg-gray-800 text-white font-semibold text-xs rounded-md transition-colors cursor-pointer shadow-2xs"
                        >
                          Apply
                        </button>
                      </div>
        ...
        {cart.length > 0 && (
          <div className="p-4 border-t border-gray-200 bg-white space-y-3">
              ...
              {discountAmount > 0 && (
                <div className="flex justify-between text-emerald-600 font-medium">
                  <span className="flex items-center gap-1">
                    <Ticket className="w-3.5 h-3.5" /> Discount {appliedVoucher?.voucher?.code ? `(${appliedVoucher.voucher.code})` : ''}:
                  </span>
            ...
            {/* Stock / Sold out error banner */}
            {formErrors.stock && (
              <div className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-md px-3.5 py-2.5" role="alert">
                <span className="text-base leading-none">⚠️</span>
                <div>
                  <p className="text-xs font-bold text-primary">Item Unavailable</p>
                  <p className="text-xs font-medium text-primary/80 mt-0.5">{formErrors.stock}</p>
                </div>
              </div>
            )}
