...
import { checkSizeAvailability } from '../../utils/productUtils';
...
export default function ProductCustomizerModal({
  ...
  const [selectedSize, setSelectedSize] = useState(() => {
    if (product?.has_sizes && product.sizes?.length > 0) {
      const available = product.sizes.find(s => checkSizeAvailability(product, s, ingredients));
  ...
  useEffect(() => {
    if (product?.has_sizes && product.sizes?.length > 0) {
      const isCurrentValid = selectedSize && 
        product.sizes.some(s => s.name === selectedSize.name && checkSizeAvailability(product, s, ingredients));
      if (!isCurrentValid) {
        const firstAvailable = product.sizes.find(s => checkSizeAvailability(product, s, ingredients));
    ...
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
      <div className="bg-white rounded-2xl sm:rounded-3xl border border-gray-150 shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh] animate-scale-up">
          ...
          {product.has_sizes && product.sizes?.length > 0 && (
            <div className="space-y-2.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                {product.sizes.map((s) => {
                  const isAvailable = checkSizeAvailability(product, s, ingredients);
                  const isSelected = selectedSize?.name === s.name;
                  return (
                    <label
                      key={s.name}
                      onClick={() => {
                        if (isAvailable) setSelectedSize(s);
                      }}
                      className={`flex items-center justify-between p-3 rounded-xl border transition-all ${
                        !isAvailable
                          ? 'opacity-45 cursor-not-allowed bg-gray-50 border-gray-200'
                          : isSelected
                          ? 'border-primary bg-primary/5 shadow-2xs font-semibold cursor-pointer'
                          : 'border-gray-200 hover:border-gray-300 bg-white font-medium cursor-pointer'
                      }`}
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        <div className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0 ${
                          !isAvailable ? 'border-gray-300' : isSelected ? 'border-primary' : 'border-gray-300'
                        }`}>
                          {isSelected && isAvailable && <div className="w-2 h-2 bg-primary rounded-full" />}
                        </div>
                        <div className="min-w-0">
                          <span className="text-xs text-gray-900 leading-tight truncate block">{s.name}</span>
                          {!isAvailable && (
                            <span className="text-[9px] font-black text-red-500 uppercase tracking-wider block">
                              Unavailable
                            </span>
                          )}
                        </div>
                      </div>
                      <span className="text-[11px] text-gray-500 shrink-0 ml-1">Rs. {s.price}</span>
                    </label>
                  );
                })}
              </div>
        ...
        <div className="p-4 sm:p-5 border-t border-gray-150 bg-white flex items-center gap-3 shrink-0">
          ...
          {(() => {
            const isCurrentSizeAvailable = !product.has_sizes || !selectedSize || checkSizeAvailability(product, selectedSize, ingredients);
            return (
              <button
                type="button"
                ...
                onClick={(e) => { 
                  e.preventDefault(); 
                  if (isCurrentSizeAvailable) handleSubmit(); 
                }}
                className={`flex-1 w-full font-medium py-3 rounded-md flex items-center justify-center gap-2 text-xs sm:text-sm shadow-2xs transition-all ${
                  isCurrentSizeAvailable
                    ? 'bg-primary hover:bg-primary-hover active:translate-y-[0.5px] text-white cursor-pointer border border-primary-hover/50'
                    : 'bg-gray-200 text-gray-400 cursor-not-allowed border border-gray-300'
                }`}
              >
                <span>{!isCurrentSizeAvailable ? 'Selected Size Unavailable' : isEditMode ? 'Update Item' : 'Add to Bowl'}</span>
                {isCurrentSizeAvailable && (
                  <>
