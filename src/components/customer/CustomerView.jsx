...
export default function CustomerView({
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] flex flex-col font-sans relative">
      ...
      {/* Header / Navbar */}
      <header className="bg-accent text-white px-3.5 py-3 sm:px-6 sm:py-3.5 border-b-2 border-secondary/80 flex justify-between items-center gap-2 shadow-sm sticky top-0 z-40">
        <div 
        ...
        >
          ...
          <div className="flex flex-col justify-center">
            <h1 className="text-base sm:text-xl font-bold text-secondary tracking-tight leading-none whitespace-nowrap">Mr. Kukooo</h1>
            <span className="text-[9px] sm:text-[10px] text-white/60 font-medium tracking-wide italic whitespace-nowrap mt-0.5">Lick the spoons!</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          {normalizedBranches.length > 0 && (
            <div className="hidden md:flex items-center gap-1.5 bg-white/10 px-3 py-1.5 rounded-md border border-white/20">
              <MapPin className="w-3.5 h-3.5 text-secondary" />
              <select
                value={selectedBranchId}
                onChange={(e) => setSelectedBranchId(e.target.value)}
                className="bg-transparent text-white font-medium text-xs border-none focus:outline-none pr-2 cursor-pointer"
              >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 sm:px-3.5 py-1.5 sm:py-2 bg-white/10 hover:bg-white/20 border border-white/20 text-white font-medium text-xs rounded-md shadow-2xs transition-all active:scale-[0.98] cursor-pointer whitespace-nowrap"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 sm:px-4 py-1.5 sm:py-2 bg-secondary hover:bg-secondary-hover text-accent font-semibold text-xs rounded-md shadow-2xs transition-all active:scale-[0.98] relative cursor-pointer whitespace-nowrap"
          >
            <ShoppingCart className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" /> 
            <span>Cart</span>
            {cart.length > 0 && (
              <span className="absolute -top-1.5 -right-1.5 sm:-top-2 sm:-right-2 px-1.5 py-0.5 rounded-md bg-primary text-white border border-accent text-[9px] sm:text-[10px] flex items-center justify-center font-bold shadow-2xs leading-none">
      ...
      {/* Main Content Area */}
      <main className="max-w-7xl mx-auto px-3 sm:px-6 mt-4 sm:mt-8 space-y-8 sm:space-y-12 flex-1 w-full">
        {viewMode === 'home' ? (
          <>
