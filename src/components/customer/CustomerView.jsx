...
export default function CustomerView({
  ...
  const handleAddCustomizedItem = ({ product, size, addons, quantity, notes, totalPricePerUnit }) => {
    const newCartItem = {
  ...
  const handleBannerClick = (offer) => {
    ...
    if (offer.redirect_type === 'category') {
      ...
      const targetProd = products.find(p => p.id.toString() === targetProductId.toString());
      if (targetProd) {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] flex flex-col font-sans text-accent">
      {/* Header / Navbar */}
      ...
      {/* Main Content Area */}
      <main className="max-w-7xl mx-auto px-6 mt-8 space-y-12 flex-1 w-full">
        {viewMode === 'home' ? (
          <>
              ...
              {homepageSections
                ...
                .map(section => {
                  ...
                  const items = products
                    .filter(p => p.is_available && p.homepage_sections && (p.homepage_sections.includes(section.id) || (isSpecialDealsSection && p.homepage_sections.includes('sec-special-deals'))));
